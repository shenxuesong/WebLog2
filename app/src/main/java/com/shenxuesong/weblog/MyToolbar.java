...
public class MyToolbar extends Toolbar {
    ...
    public MyToolbar(Context context, @Nullable AttributeSet attrs, int defStyleAttr) {
        ...
        if (attrs !=null){
            @SuppressLint("RestrictedApi") final TintTypedArray a = TintTypedArray.obtainStyledAttributes(getContext(),attrs,R.styleable.MyToolbar,defStyleAttr,0);

            @SuppressLint("RestrictedApi") final Drawable leftIcon = a.getDrawable(R.styleable.MyToolbar_leftIcon);
            if (leftIcon !=null){
                setLeftIcon(leftIcon);
            }

            @SuppressLint("RestrictedApi") final Drawable rightIcon = a.getDrawable(R.styleable.MyToolbar_rightIcon);
            if (rightIcon != null){
                setRightIcon(rightIcon);
            }

            @SuppressLint("RestrictedApi") final String title= a.getString(R.styleable.MyToolbar_Title);
            if (title != null){
                showTitleView();
            }
